...
export async function buildEmbeddingsSnapshot({ sucursalId = null, updatedSince = null, dim = null } = {}) {
  const whereEmp = {};
  ...
  // ✅ NUEVO: filtrar por dimensión (recomendado: 192)
  if (dim) whereEmb.dim = dim;
  ...
  const map = new Map();
  let resolvedDim = dim || null;

  for (const r of rows) {
    if (!resolvedDim && r.dim) resolvedDim = r.dim;
