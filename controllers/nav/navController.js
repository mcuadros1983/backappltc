...
// // - Fallback: si rol_id === 1 o usuario === '1' => todos los permisos.
// // =====================================================
...
// function getAllPermsFromKnownPaths() {
//   // recolecta todos los permisos que infiere inferPerm
//   const set = new Set();
//   for (const p of KNOWN_PATHS) {
//     const need = inferPerm(p);
...
// /** --- Auth / permisos --- */
// async function getUserPerms(userId, roleIdFromQuery) {
//   if (!userId) {
//     console.log("[nav] getUserPerms: sin userId => []");
//     return [];
//   }

//   const u = await Usuario.findByPk(userId, {
//     attributes: ["id", "permissions", "rol_id", "usuario"],
//   });

//   if (!u) {
//     console.log("[nav] getUserPerms: usuario no encontrado");
//     return [];
//   }

//   const rolId = Number(roleIdFromQuery ?? u.rol_id ?? 0);
//   const isSuperUser =
//     String(u.usuario) === "1" || rolId === 1; // 🔸 regla de superadmin que ya usás en otras partes

//   let raw = u.permissions ?? u.permisos ?? [];
//   console.log("[nav] DB usuario:", {
//     id: u.id,
//     rol_id: u.rol_id,
//     usuario: u.usuario,
//     hasPermissions: !!raw,
...
//   // Parseo flexible: array, JSON string o CSV
//   let parsed = [];
...
//       if (!Array.isArray(parsed)) parsed = raw.split(",").map(s => s.trim()).filter(Boolean);
//     } catch {
//       parsed = raw.split(",").map(s => s.trim()).filter(Boolean);
//     }
//   }

//   // Fallback: superadmin => todos los permisos
//   if (!parsed.length && isSuperUser) {
//     const all = getAllPermsFromKnownPaths();
//     console.log(`[nav] Fallback SUPERADMIN: rol_id=${rolId} / usuario=${u.usuario} => ${all.length} permisos`);
//     return all;
//   }

//   console.log(`[nav] permisos usuario parseados: ${parsed.length}`);
//   return parsed;
// }
...
//     const userPerms = await getUserPerms(userId, roleId);
