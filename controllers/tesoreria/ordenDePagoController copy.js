...
export const emitirOrdenPago = async (req, res) => {
  ...
  try {
    ...
    // No permitir cta cte como medio
    for (const p of pagos) {
      const medio = String(p.medio || "").toLowerCase();
      if (medio === "ctacte" || /cta\.?\s*cte|cuenta\s*corriente/i.test(medio)) {
        throw new Error("El medio 'cta cte' no está permitido para emitir la orden de pago.");
      }
    }
    ...
    if (totalAplicado < 0) throw new Error("Total aplicado inválido");
    ...
    // Si solo hay un comprobante, lo seteamos en la OP; si hay varios queda null
    ...
    const orden = await OrdenPago.create(
      {
        empresa_id,
        proveedor_id,
        comprobanteegreso_id: singleCompId, // <- clave del cambio
        fecha: fechaOrden,
        total: totalPagos,
        estado: "emitida",
        numero: null,
    ...
    // dentro de emitirOrdenPago, luego de haber armado `byId` y antes de crear movimientos:
    const getCompLabel = () => {
      const uniques = [...new Set(items.map(i => Number(i.comprobanteegreso_id)).filter(Boolean))];
      if (uniques.length === 0) return null;
      if (uniques.length === 1) {
        const c = byId[uniques[0]];
        const nro = c?.nrocomprobante || c?.id;
        return `Comp. ${nro}`;
      }
      return `Varios comprobantes (${uniques.length})`;
    };
    const compLabel = getCompLabel();
    const withComp = (base) => compLabel ? `${base} (${compLabel})` : base;


    // Crear movimientos por medio de pago (todos con ordenpago_id)
    for (const p of pagos) {
      const medio = String(p.medio || "").toLowerCase();
      ...
      if (medio === "echeq" || /e-?\s*cheq|echeq/i.test(medio)) {
        ...
        await EcheqEmitido.create(
          {
            comprobanteegreso_id: singleCompId || null, // opcional: si hay uno, lo guardamos
            proveedor_id,
            empresa_id,
    ...
    // Aplicar abonos (cta cte) y actualizar comprobantes
    ...
    for (const it of items) {
      ...
      await comp.update(
        { saldo: nuevoSaldo, estadopago: nuevoEstado },
        { transaction: t }
      );
      ...
      await MovimientoCtaCteProveedor.create(
        {
          ...
          fecha: fechaOrden,
          fecha_pago: fechaOrden,
          descripcion: `Saldo a favor por Orden de Pago #${orden.id}`,
          tipo: "abono",
          importe: exceso,
          origen_tipo: "OrdenPago",
          origen_id: orden.id,
