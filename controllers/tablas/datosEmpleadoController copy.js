...
// Helpers
const is1to7orNull = (x) => x == null || (Number.isInteger(Number(x)) && Number(x) >= 1 && Number(x) <= 7);
const telefonoOk = (x) => x == null || /^[+()\-. \s0-9]+$/.test(String(x));
const enteroPositivoONull = (x) => x == null || (Number.isInteger(Number(x)) && Number(x) > 0);
...
export const upsertPorEmpleado = async (req, res) => {
  ...
  try {
    ...
    // Armamos "changes" SOLO con las claves presentes en el body.
    const changes = {};
    ...
    if (Object.prototype.hasOwnProperty.call(req.body, "turno_id")) {
      const v = req.body.turno_id;
      changes.turno_id = (v === null || v === "" ? null : Number(v));
    }
    ...
    // Validaciones SOLO sobre lo que se pretende cambiar
    if (Object.prototype.hasOwnProperty.call(changes, "franco_am") && !is1to7orNull(changes.franco_am)) {
      await t.rollback();
      return res.status(400).json({ error: "franco_am debe estar entre 1 y 7 o ser null" });
    }
    if (Object.prototype.hasOwnProperty.call(changes, "franco_pm") && !is1to7orNull(changes.franco_pm)) {
      await t.rollback();
      return res.status(400).json({ error: "franco_pm debe estar entre 1 y 7 o ser null" });
    }
    if (Object.prototype.hasOwnProperty.call(changes, "telefono") && !telefonoOk(changes.telefono)) {
      await t.rollback();
      return res.status(400).json({ error: "Formato de teléfono inválido" });
    }
    if (Object.prototype.hasOwnProperty.call(changes, "turno_id") && !enteroPositivoONull(changes.turno_id)) {
      await t.rollback();
      return res.status(400).json({ error: "turno_id debe ser un entero positivo o null" });
    }

    // Upsert 1:1
    const [row, created] = await DatosEmpleado.findOrCreate({
      where: { empleado_id },
      defaults: { empleado_id, ...changes }, // si no existe, crear con lo que llegó
      transaction: t,
      lock: t.LOCK.UPDATE,
    });

    // Si ya existía y hay cambios, actualizá solo esos campos
    ...
    return res.status(500).json({ error: "Error guardando datos del empleado", detalle: error.message });
  }
};
...
export const obtenerPorEmpleado = async (req, res) => {
  try {
    ...
    if (!row) return res.status(404).json({ error: "Datos del empleado no encontrados" });
...
export const eliminarPorEmpleado = async (req, res) => {
  ...
  try {
    ...
    if (!row) {
      await t.rollback();
      return res.status(404).json({ error: "Datos del empleado no encontrados" });
    }
    await row.destroy({ transaction: t });
    await t.commit();
    return res.status(200).json({ mensaje: "Datos del empleado eliminados" });
  } catch (error) {
    await t.rollback();
    console.error("❌ eliminarPorEmpleado:", error);
    return res.status(500).json({ error: "Error al eliminar los datos del empleado", detalle: error.message });
  }
};

// Listado opcional por filtros (sucursal_id, turno_id, paginación)
export const listar = async (req, res) => {
  try {
    ...
    if (req.query.turno_id) where.turno_id = Number(req.query.turno_id); // 👈 nuevo filtro opcional
    ...
    return res.status(500).json({ error: "Error al listar datos de empleados", detalle: error.message });
  }
};
